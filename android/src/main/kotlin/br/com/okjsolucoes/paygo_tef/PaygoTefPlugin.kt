...
class PaygoTefPlugin(private val context: Context) : MethodCallHandler {

    companion object {
        private const val CHANNEL = "br.com.okjsolucoes.paygo_tef"

        // Método para registrar o plugin no FlutterEngine
        fun registerWith(flutterEngine: FlutterEngine, context: Context) {
            val channel = MethodChannel(flutterEngine.dartExecutor.binaryMessenger, CHANNEL)
            val instance = PaygoTefPlugin(context)
            channel.setMethodCallHandler(instance)
            Log.d("PaygoTefPlugin", "Plugin registrado no canal: $CHANNEL")
        }
    }
    ...
    override fun onDetachedFromEngine(binding: FlutterPluginBinding) {
        ...
        // Aqui você pode delegar para a mesma lógica que já escreveu
        handleMethodCall(call, result)
    }


    // Recebe chamadas do Flutter
    override fun handleMethodCall(call: MethodCall, result: Result) {
        Log.d("PaygoTefPlugin", "Método chamado: ${call.method}")
        ...
        when (call.method) {
            ...
            "dadosAutomacao" -> {
                ...
                try {
                    ...
                    result.success("Dados da automação criados com sucesso. Transação Iniciada!") // result.success(true)
                } catch (e: Exception) {
                ...
                CoroutineScope(Dispatchers.IO).launch {
                    try {
                        ...
                        if (operacao == Operacoes.VENDA) {
                            val entrada = EntradaTransacaoHelper.operacaoVenda(
                                identificadorTransacaoAutomacao = identificadorTransacao,
                                operacao = operacao,
                                ...
                                if (saida != null) {
                                    ...
                                    if (dadosSaida["necessitaConfirmacao"] as Boolean) {
                                        val confirmacao = Confirmacoes()
                                            ...
                                            .informaStatusTransacao(StatusTransacao.CONFIRMADO_AUTOMATICO)
...
//                                                "Erro ao confirmar transação: ${e.message ?: e.toString()}",
//                                                null
//                                            )
                                        }
                                    }
                                    ...
                                    //aqui não posso usar dadosSaida["necessitaConfirmacao"] as Boolean
                                    //pois estou capturando novo resultado após confirmação ou pendente
                                    //eventualmente serem acionados, mudando o status inicial
                                    if (saida.obtemResultadoTransacao() == 0) {
                                        //impressão do comprovante
                                        val listComprovanteDifLoja: List<String> =
                                            saida.obtemComprovanteDiferenciadoLoja() ?: emptyList()
                                        Log.e(
                                            "PaygoTefPlugin",
                                        ...
                                        val listComprovanteReduzidoPortador: List<String> =
                                            saida.obtemComprovanteReduzidoPortador() ?: emptyList()
                                        ...
                                        val retorno = mapOf(
                                            "status" to "sucess",
                                            "mensagem" to "Operação VENDA realizada com sucesso.",
                                            "identificadorTransacao" to entrada.obtemIdTransacaoAutomacao(),
                                            "valorTotal" to entrada.obtemValorTotal(),
                                            "modalidadePagamento" to entrada.obtemModalidadePagamento().name,
                                            "numeroParcelas" to entrada.obtemNumeroParcelas(),
                                            "codigoMoeda" to entrada.obtemCodigoMoeda(),
                                            "documentoFiscal" to entrada.obtemDocumentoFiscal(),
                                            "estabelecimento" to entrada.obtemEstabelecimentoCNPJouCPF(),
                                            "campoLivre" to entrada.obtemDadosAdicionaisAutomacao1(),
                                            "idTransacao" to identificadorTransacao,
                                            "idConfirmacaoTransacao" to idConfirmacaoTransacao,
                                            "mensagem_saida" to dadosSaida["mensagemResultado"],
                                            "resultadoTransacao" to dadosSaida["resultadoTransacao"],
                                            // impressão do comprovante
                                            "comprovanteGraficoPortador" to stringComprovanteGrafPortador,
                                            "comprovanteCompletoList" to listComprovanteCompleto,
                                            "comprovanteReduzidoPortadorList" to listComprovanteReduzidoPortador,
                                            "comprovanteDifLojaList" to listComprovanteDifLoja,
                                            "comprovanteGrafLojista" to stringComprovanteGrafLojista,
                                            "comprovanteDifPortadorList" to listComprovanteDifPortador,
                                        )
                                        ...
                                        val retorno = mapOf(
                                            "status" to "error",
                                            "mensagem" to "Operação VENDA falhou!",
                                            "identificadorTransacao" to entrada.obtemIdTransacaoAutomacao(),
                                            "valorTotal" to entrada.obtemValorTotal(),
                                            "modalidadePagamento" to entrada.obtemModalidadePagamento().name,
                                            "numeroParcelas" to entrada.obtemNumeroParcelas(),
                                ...
                                withContext(Dispatchers.Main) {
                                    result.error(
                                        "SAIDA_TRANSACAO_ERROR",
                                        e.message
                                            ?: "${e.toString()}",//"Erro ao receber saida da transação de Versão. Saída Nula",
                                        e.stackTraceToString()
                                    )
                                }
                            }
                            ...
                            val entrada = EntradaTransacaoHelper.operacaoExibePdc(
                                identificadorTransacaoAutomacao = identificadorTransacao,
                                operacao = operacao,
                            )
                            ...
                            try {
                                ...
                                if (saida != null) {
                                    ...
                                    if (saida.obtemResultadoTransacao() == 0) {
                                        val retorno = mapOf(
                                            "status" to "sucess",
                                            "mensagem" to "Operação EXIBE_PDC realizada com sucesso.",
                                            "idTransacao" to identificadorTransacao,
                                            "idConfirmacaoTransacao" to idConfirmacaoTransacao,
                                            "mensagem_saida" to dadosSaida["mensagemResultado"],
                                            "resultadoTransacao" to dadosSaida["resultadoTransacao"],
                                        ...
                                        val retorno = mapOf(
                                            "status" to "error",
                                            "mensagem" to "Operação EXIBE_PDC falhou!",
                                            "idTransacao" to identificadorTransacao,
                                            "idConfirmacaoTransacao" to idConfirmacaoTransacao,
                                            "mensagem_saida" to dadosSaida["mensagemResultado"],
                                            "resultadoTransacao" to dadosSaida["resultadoTransacao"],
                            ...
                            val entrada = EntradaTransacaoHelper.operacaoVersao(
                                identificadorTransacaoAutomacao = identificadorTransacao,
                                operacao = operacao,
                            )
                            ...
                            try {
                                ...
                                if (saida != null) {
                                    ...
                                    if (saida.obtemResultadoTransacao() == 0) {
                                        val retorno = mapOf(
                                            "status" to "sucess",
                                            "mensagem" to "Operação VERSAO realizada com sucesso",
                                            "idConfirmacaoTransacao" to idConfirmacaoTransacao,
                                        ...
                                        val retorno = mapOf(
                                            "status" to "error",
                                            "mensagem" to "Operação VERSAO falhou!",
                                            "idConfirmacaoTransacao" to idConfirmacaoTransacao,
                                            "versaoApk" to versaoAPK,
                                            "versaoLib" to versaoBiblioteca,
                                            "mensagem_saida" to dadosSaida["mensagemResultado"],
                            ...
                            val entrada = EntradaTransacaoHelper.operacaoCancelamento(
                                identificadorTransacaoAutomacao = identificadorTransacao,
                                operacao = operacao,
                                modalidadePagamento = modalidadePagamento,
                                tipoCartao = tipoCartao,
                            ...
                            try {
                                ...
                                if (saida != null) {
                                    ...
                                    Log.d(
                                        ...
                                        - Identificador: $identificadorTransacao
                                        - idConfirmacao: $idConfirmacaoTransacao
                                        - Mensagem: Entrada de transação (operacaoCancecrilamento) criada com sucesso
                                    ...
                                    if (saida.obtemResultadoTransacao() == 0) {
                                        ...
                                        val retorno = mapOf(
                                            "status" to "sucess",
                                            "mensagem" to "Operação CANCELAMENTO criada com sucesso.",
                                            "identificadorTransacao" to entrada.obtemIdTransacaoAutomacao(),
                                            "valorTotal" to entrada.obtemValorTotal(),
                                            "modalidadePagamento" to entrada.obtemModalidadePagamento().name,
                                            "numeroParcelas" to entrada.obtemNumeroParcelas(),
                                        ...
                                        val retorno = mapOf(
                                            "status" to "error",
                                            "mensagem" to "Operação CANCELAMENTO falhou!",
                                            "identificadorTransacao" to entrada.obtemIdTransacaoAutomacao(),
                                            "valorTotal" to entrada.obtemValorTotal(),
                                            "modalidadePagamento" to entrada.obtemModalidadePagamento().name,
                                            "numeroParcelas" to entrada.obtemNumeroParcelas(),
